...
if (options.includes('-version') || options.includes('-v')) {
  const version = execSync('npm ls -g @moonstone78/npmaudit', { encoding: 'utf-8' });
  console.log(chalk.green(' V'+ version.substring(version.lastIndexOf("@") +1)));
} else if (options.includes('-latest') || options.includes('-l')) {
  const version = execSync('npm view @moonstone78/npmaudit version', { encoding: 'utf-8' });
